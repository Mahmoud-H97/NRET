...
def get_spectra():
    ...
    # PROSPECT 5
    prospect_5_spectraf = pkgutil.get_data("nret", "prospect5_spectra.txt")
    ...
    # SOIL
    soil_spectraf = pkgutil.get_data("nret", "soil_reflectance.txt")
    rsoil1, rsoil2 = np.loadtxt(BytesIO(soil_spectraf), unpack=True)
    soil_spectra = SoilSpectra(rsoil1, rsoil2)
    # LIGHT
    light_spectraf = pkgutil.get_data("nret", "light_spectra.txt")
